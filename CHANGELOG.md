...
- 修正 1.20.1 被误判为组件编解码服务端的问题，恢复 1.20.5 组件版本边界。
- 为组件化之前的新 NMS 包名及映射后的物品保存方法补充兼容解析，保留完整物品 NBT。
- 增加版本边界和方法解析回归测试；PRTS/Arclight 实服保存仍待验证。

## 3.0.1 (2026-09-07)

- 发布正式版，统一插件版本、Gradle 产物和 API 坐标为 `3.0.1`。
- 收录已验证的 SX 联动、多版本兼容、MySQL 支持、中文默认配置及 Redis 可选缓存实现。
- 本次版本发布不改变运行逻辑；构建产物与缓存预发行版的逐条目核验见 [发布记录](docs/RELEASE-3.0.1.md)。
