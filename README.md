...
当前正式版为 [3.0.2](https://github.com/FxRayHughes/SX-RPGInventory/releases/tag/v3.0.2)，修正 1.20.1 物品 NBT 编解码路径，支持 MySQL 和默认中文配置，Redis 仅作为 SQL 的可选缓存。验证范围见 [3.0.2 发布记录](docs/RELEASE-3.0.2.md)，PRTS/Arclight 实服验证尚未完成。[Redis 缓存版本](docs/REDIS-CACHE.md)、[MySQL 与中文更新](docs/MYSQL-CHINESE.md) 和 [服务器矩阵](docs/SERVER-MATRIX.md) 保留为历史证据，不同产物的结果分开记录。
...
Linux/macOS 使用 `./gradlew`。默认 Paper API 为 `26.1.2.build.74-stable`。插件产物为 `build/libs/SX-RPGInventory-3.0.2.jar`；`-plain.jar` 不包含运行库，不用于服务器安装。
...
`api` publication 的坐标为 `github.saukiya.sxrpginventory:sx-rpginventory:3.0.2`，发布普通薄 JAR 和源码。薄包本地产物为 `build/libs/SX-RPGInventory-api-3.0.2.jar`；POM 保留真实运行时依赖，Bukkit 和可选服务器 API 标记为 provided。消费插件可使用 `compileOnly('github.saukiya.sxrpginventory:sx-rpginventory:3.0.2') { transitive = false }` 并声明自己的目标 Bukkit API。
